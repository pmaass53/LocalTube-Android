...
export default class CommentView extends YTNode {
    ...
    async translate(target_language) {
        ...
        // Using new RegExp to avoid parse-time SyntaxError in environments without full Unicode property support
        let text;
        try {
            text = this.content.toString().replace(new RegExp('[^\\p{L}\\p{N}\\p{P}\\p{Z}]', 'gu'), '');
        } catch (e) {
            text = this.content.toString().replace(/[^\x00-\x7F]/g, '');
        }
