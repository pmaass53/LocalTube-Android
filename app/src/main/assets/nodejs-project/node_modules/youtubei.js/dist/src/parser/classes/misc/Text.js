    ...
    static processAttachmentRuns(runs, attachment_runs, data) {
        for (const attachment_run of attachment_runs) {
            ...
            if (attachment_run.length === 0) {
                ...
                // Fallback regex for environments where \p{Emoji} is not supported (like some Node.js builds for Android)
                const is_emoji = (/^(?:[\uD800-\uDBFF][\uDC00-\uDFFF]|\u200d)+$/).test(text);
                if (attachment_run.element?.type?.imageType?.image && (is_custom_emoji || is_emoji)) {
                    const emoji = {
